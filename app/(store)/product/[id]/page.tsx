...
export default function ProductDetailPage() {
  ...
  if (loading) return <div style={{ padding: '100px', textAlign: 'center', color: '#94a3b8', fontWeight: 'bold' }}>Loading product details...</div>;
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(400px, 1fr))', gap: '80px' }}>
        ...
        <div>
          <div style={{ position: 'relative', borderRadius: '40px', overflow: 'hidden', backgroundColor: 'white', border: '8px solid white', boxShadow: '0 25px 50px -12px rgba(0, 0, 0, 0.1)' }}>
            <img 
              src={product.image || 'https://images.unsplash.com/photo-1599021419847-d8a7a6ac599d?q=80&w=1000'} 
              alt={product.name} 
        ...
        <div style={{ display: 'flex', flexDirection: 'column' }}>
          <div style={{ marginBottom: '32px' }}>
            <span style={{ fontSize: '0.7rem', fontWeight: '900', color: '#059669', textTransform: 'uppercase', letterSpacing: '0.2em' }}>Handcrafted Heritage</span>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '16px', marginBottom: '40px' }}>
            <span style={{ fontSize: '3rem', fontWeight: '900', color: '#059669' }}>₹{currentPrice.toFixed(2)}</span>
            {selectedWeight !== '250g' && (
              <span style={{ background: '#ecfdf5', color: '#059669', padding: '4px 12px', borderRadius: '8px', fontSize: '0.75rem', fontWeight: '800' }}>BEST VALUE</span>
            )}
          </div>
              ...
              {weights.map((w) => (
                <button
                  ...
                  style={{
                    ...
                    borderRadius: '16px',
                    border: '2px solid',
                    borderColor: selectedWeight === w ? '#059669' : '#f1f5f9',
                    background: selectedWeight === w ? '#059669' : 'white',
