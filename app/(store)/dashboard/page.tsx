  ...
  return (
    <div style={{ maxWidth: '1100px', margin: '0 auto', padding: '40px 20px', fontFamily: 'Fraunces, serif' }}>
      ...
      <div style={{ marginBottom: '60px' }}>
        <h1 style={{ fontSize: 'clamp(2rem, 5vw, 3.5rem)', fontWeight: '900', color: '#1e293b', margin: 0, fontFamily: 'Fraunces, serif' }}>
          Hello, <span style={{ color: '#2d5a27' }}>{user.phone}</span>
        </h1>
        <p style={{ color: '#64748b', marginTop: '12px', fontWeight: '500' }}>Welcome back to your dashboard.</p>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(320px, 1fr))', gap: '30px' }}>
        ...
        <div style={{ background: '#0f172a', borderRadius: '40px', padding: '40px', color: 'white', boxShadow: '0 25px 50px -12px rgba(15, 23, 42, 0.3)', position: 'relative', overflow: 'hidden' }}>
          ...
          <div style={{ marginTop: '40px', display: 'flex', gap: '12px' }}>
            <button onClick={() => setIsModalOpen(true)} style={{ flex: 1, background: '#2d5a27', color: 'white', padding: '16px', borderRadius: '18px', border: 'none', fontWeight: '900', cursor: 'pointer', transition: '0.2s' }}>Add Funds</button>
        ...
        <Link href="/orders" style={{ textDecoration: 'none', background: 'white', borderRadius: '40px', padding: '40px', border: '1px solid #f1f5f9', display: 'flex', flexDirection: 'column', gap: '20px', transition: 'transform 0.3s, box-shadow 0.3s' }}>
          <div style={{ width: '56px', height: '56px', background: '#f0f7f0', borderRadius: '18px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><ShoppingBag size={28} color="#2d5a27" /></div>
          <div>
            <h3 style={{ fontSize: '1.5rem', fontWeight: '800', color: '#1e293b', margin: '0 0 8px 0', fontFamily: 'Fraunces, serif' }}>My Orders</h3>
            <p style={{ color: '#64748b', fontSize: '0.9rem' }}>Track and manage your pickle history.</p>
          </div>
          <div style={{ marginTop: 'auto', display: 'flex', alignItems: 'center', gap: '8px', color: '#2d5a27', fontWeight: '800' }}>Manage Orders <ArrowRight size={18} /></div>
        </Link>
      </div>

      {/* Sign Out */}
      <div style={{ textAlign: 'center', marginTop: '50px', paddingTop: '40px', borderTop: '1px solid #f1f5f9' }}>
        <button
          onClick={() => { logout(); router.push('/'); }}
          style={{ background: '#f0f7f0', color: '#b91c1c', border: '2px solid #e8f1e8', padding: '14px 36px', borderRadius: '18px', fontWeight: '800', fontSize: '0.95rem', cursor: 'pointer', transition: 'all 0.2s', fontFamily: 'Fraunces, serif', display: 'inline-flex', alignItems: 'center', gap: '8px' }}
          ...
          onMouseLeave={e => { (e.currentTarget as HTMLElement).style.background = '#f0f7f0'; (e.currentTarget as HTMLElement).style.borderColor = '#e8f1e8'; }}
        >
          Sign Out
        </button>
      </div>
      ...
      {isModalOpen && (
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.6)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, padding: '20px', backdropFilter: 'blur(4px)' }}>
          <div style={{ background: 'white', padding: '40px', borderRadius: '40px', width: '100%', maxWidth: '450px', position: 'relative' }}>
            ...
            <h2 style={{ fontSize: '2rem', fontWeight: '900', color: '#1e293b', marginBottom: '12px', fontFamily: 'Fraunces, serif' }}>Add Funds</h2>
            ...
            {!isRazorpayLoaded && (
              <p style={{ fontSize: '0.8rem', color: '#94a3b8', textAlign: 'center', marginBottom: '16px', fontWeight: '600' }}>
            ...
            <button
              ...
              style={{ width: '100%', background: isRazorpayLoaded ? '#2d5a27' : '#94a3b8', color: 'white', padding: '20px', borderRadius: '22px', border: 'none', fontWeight: '900', fontSize: '1.1rem', cursor: (paymentLoading || !isRazorpayLoaded) ? 'not-allowed' : 'pointer', boxShadow: '0 10px 15px -3px rgba(45, 90, 39, 0.2)', opacity: (paymentLoading || !isRazorpayLoaded) ? 0.7 : 1, transition: 'all 0.3s' }}
            >
