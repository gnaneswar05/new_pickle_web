    ...
    <div>
      ...
      {loading ? (
        ...
        <section style={{ position: 'relative', overflow: 'hidden', padding: '120px 20px', background: '#0f172a', minHeight: '80vh', display: 'flex', alignItems: 'center' }}>
          ...
          {/* Glowing Orbs */}
          <div style={{ position: 'absolute', top: '-20%', left: '-10%', width: '600px', height: '600px', background: 'var(--primary)', filter: 'blur(150px)', opacity: 0.25, borderRadius: '50%' }}></div>
          <div style={{ position: 'absolute', bottom: '-20%', right: '-10%', width: '600px', height: '600px', background: 'var(--primary)', filter: 'blur(150px)', opacity: 0.1, borderRadius: '50%' }}></div>
          ...
          <div className="container" style={{ position: 'relative', zIndex: 10, display: 'flex', alignItems: 'center', gap: '80px', flexWrap: 'wrap' }}>
            <div style={{ flex: '1 1 500px' }} className="fade-transition" key={activeSlide?._id || 'default'}>
              <div style={{ display: 'inline-block', padding: '10px 20px', background: 'var(--secondary)', color: 'var(--primary)', borderRadius: '30px', fontSize: '0.85rem', fontWeight: '900', letterSpacing: '0.15em', textTransform: 'uppercase', marginBottom: '30px', border: '1px solid var(--border)' }}>
              ...
              <h1 style={{ fontSize: 'clamp(3.5rem, 6vw, 5.5rem)', fontWeight: '900', color: 'white', lineHeight: 1.1, fontFamily: 'Fraunces, serif', marginBottom: '30px' }}>
                {activeSlide?.title ? (
                  <>
                    ...
                    <span style={{ color: 'var(--primary)', fontStyle: 'italic' }}>{activeSlide.title.split(' ').slice(-1)}</span>
                  </>
                ) : (
                  <>Taste the <span style={{ color: 'var(--primary)', fontStyle: 'italic' }}>Tradition</span> of India.</>
                )}
              </h1>
              ...
              <div style={{ display: 'flex', gap: '20px', flexWrap: 'wrap' }}>
                <Link href={activeSlide?.link || "/products"} className="hero-btn" style={{ background: 'var(--primary)', color: 'white', padding: '22px 45px', borderRadius: '30px', textDecoration: 'none', fontWeight: '900', fontSize: '1.1rem', display: 'flex', alignItems: 'center', gap: '12px', boxShadow: '0 20px 25px -5px rgba(220, 38, 38, 0.3)', transition: 'all 0.3s' }}>
              ...
              <div className="hero-img fade-transition" key={activeSlide?.image || 'img'} style={{ position: 'relative', width: '100%', maxWidth: '450px', aspectRatio: '4/5', borderRadius: '48px', overflow: 'hidden', boxShadow: '0 30px 60px -15px rgba(0,0,0,0.8)', transform: 'rotate(2deg)' }}>
                ...
                <div style={{ position: 'absolute', bottom: '40px', left: '40px', right: '40px' }}>
                  <div style={{ background: 'rgba(255,255,255,0.1)', backdropFilter: 'blur(20px)', padding: '20px', borderRadius: '24px', border: '1px solid rgba(255,255,255,0.2)', display: 'flex', alignItems: 'center', gap: '15px' }}>
                    <div style={{ width: '50px', height: '50px', background: 'var(--primary)', borderRadius: '16px', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'white' }}>
                  ...
                  {sliders.map((_, idx) => (
                    <button 
                      ...
                      style={{ 
                        ...
                        background: currentSlideIndex === idx ? 'var(--primary)' : 'rgba(255,255,255,0.2)',
      ...
      <section className="container" style={{ padding: '6rem 20px' }}>
        <h2 style={{ fontSize: '3rem', fontWeight: '900', color: 'var(--text-main)', marginBottom: '4rem', fontFamily: 'Fraunces, serif', textAlign: 'center' }}>
            ...
            categories.map((cat: any) => (
              ...
              >
                ...
                onMouseOut={(e) => {
                  ...
                  (e.currentTarget as HTMLElement).style.borderColor = 'var(--surface)';
                }}
                >
                  <img 
                ...
                <h3 style={{ fontSize: '1.25rem', fontWeight: '800', color: 'var(--text-main)', margin: 0, fontFamily: 'Fraunces, serif', textAlign: 'center' }}>{cat.name}</h3>
              </Link>
            ))
          )}
        </div>
      ...
      <section style={{ background: 'var(--secondary)', padding: '6rem 20px' }}>
        <div className="container">
          <h2 className="text-4xl text-center" style={{ marginBottom: '4rem', color: 'var(--text-main)' }}>Our Top Selling Delicacies</h2>
      ...
      <section style={{ 
        background: 'linear-gradient(180deg, #450a0a 0%, #0f0505 100%)', 
        color: 'white', 
        padding: '100px 20px',
        position: 'relative',
        overflow: 'hidden'
        ...
        {/* Glow Effects */}
        <div style={{ position: 'absolute', top: '10%', left: '50%', transform: 'translateX(-50%)', width: '400px', height: '400px', background: 'var(--primary)', filter: 'blur(150px)', opacity: 0.15, borderRadius: '50%', pointerEvents: 'none' }}></div>

        <div className="container" style={{ position: 'relative', zIndex: 2 }}>
          <div style={{ textAlign: 'center', marginBottom: '60px' }}>
            <span style={{ fontSize: '0.85rem', fontWeight: '800', textTransform: 'uppercase', letterSpacing: '0.25em', color: 'var(--primary)', display: 'inline-block', marginBottom: '12px' }}>
              The Kanvi Promise
            </span>
