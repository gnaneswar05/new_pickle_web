...
export default function CategoriesPage() {
  ...
  return (
    <div className="container" style={{ padding: '60px 20px', minHeight: '70vh' }}>
      <h1 style={{ fontSize: '3.5rem', fontWeight: '900', color: '#1e293b', marginBottom: '1.5rem', fontFamily: 'Fraunces, serif', textAlign: 'center' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: '50px', justifyItems: 'center' }}>
        {categories.map((cat: any) => (
          <Link href={`/products?category=${cat.name}`} key={cat._id} className="group" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', textDecoration: 'none' }}>
            ...
            <h2 style={{ fontSize: '1.5rem', fontWeight: '800', color: '#1e293b', margin: 0, fontFamily: 'Fraunces, serif', textAlign: 'center' }}>{cat.name}</h2>
          </Link>
        ))}
      </div>
    </div>
