...
export default function WalletPage() {
  ...
  return (
    <div style={{ minHeight: '90vh', background: '#fafbfc', padding: '60px 20px', fontFamily: 'Fraunces, serif' }}>
      <div style={{ maxWidth: '900px', margin: '0 auto' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginBottom: '50px', gap: '20px' }}>
          <div>
            <h1 style={{ fontSize: 'clamp(2.5rem, 6vw, 3.5rem)', fontWeight: '900', color: '#1e293b', margin: 0, fontFamily: 'Fraunces, serif', lineHeight: 1.1 }}>
              <span style={{ color: '#2d5a27' }}>Wallet</span>
            </h1>
            <p style={{ color: '#64748b', marginTop: '12px', fontWeight: '500', fontSize: '1.1rem' }}>Manage your funds for seamless shopping.</p>
          </div>
          <Link href="/dashboard" style={{ background: '#2d5a27', color: 'white', padding: '16px 28px', borderRadius: '18px', textDecoration: 'none', fontWeight: '800', fontSize: '0.9rem', display: 'flex', alignItems: 'center', gap: '10px', boxShadow: '0 10px 20px -5px rgba(45, 90, 39, 0.3)', transition: '0.3s' }}>
            Back
          </Link>
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '40px', alignItems: 'flex-start' }}>
          ...
          <div style={{ position: 'sticky', top: '100px' }}>
            <div style={{ background: 'linear-gradient(135deg, #2d5a27 0%, #142911 100%)', color: 'white', padding: '50px', borderRadius: '48px', position: 'relative', overflow: 'hidden', boxShadow: '0 30px 60px -15px rgba(45, 90, 39, 0.4)' }}>
          ...
          <div style={{ background: 'white', padding: '50px 40px', borderRadius: '48px', border: '1px solid #f1f5f9', boxShadow: '0 10px 25px -5px rgba(0,0,0,0.02)' }}>
            <h3 style={{ fontSize: '1.5rem', fontWeight: '800', color: '#1e293b', marginBottom: '35px', display: 'flex', alignItems: 'center', gap: '12px', fontFamily: 'Fraunces, serif' }}>
              <History size={24} color="#2d5a27" /> Transaction History
            </h3>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
                {data.transactions.map((t: any) => (
                  <div key={t._id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '20px', borderRadius: '24px', border: '1px solid #f8fafc', background: '#fafbfc' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '18px' }}>
                      <div style={{ width: '45px', height: '45px', background: t.type === 'Credit' ? '#f0f7f0' : '#f0f7f0', color: t.type === 'Credit' ? '#2d5a27' : '#b91c1c', borderRadius: '14px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                    ...
                    <div style={{ textAlign: 'right' }}>
                      <p style={{ margin: 0, fontWeight: '900', color: t.type === 'Credit' ? '#2d5a27' : '#b91c1c', fontSize: '1.1rem' }}>
                ...
                {data.transactions.length === 0 && (
                  <div style={{ textAlign: 'center', padding: '60px 0' }}>
                    ...
                    <p style={{ color: '#94a3b8', fontWeight: '600', fontSize: '0.9rem' }}>No transactions yet.</p>
                  </div>
                )}
